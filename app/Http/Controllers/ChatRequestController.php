    ...
    public function update_chat_request(Request $request, $id)
    {
        ...
        if ($chat_request) {
            if ($request->input('action') === 'accept') {
                ...
                $session_model->where('user_id', $request->input("customer_id"))->update(['chatting_with' => auth()->id()]);
                ...
                $chat_request->ongoing = 2;
                 ...
                 $session_model->where('user_id', $request->input("customer_id"))->update(['chatting_with' => 0,"live_chat"=>0]);
